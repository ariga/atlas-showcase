...
CREATE TABLE "public"."users" (
  ...
  CONSTRAINT "users_username_normalized" CHECK (username = lower(btrim(username)) AND length(btrim(username)) > 0),
  CONSTRAINT "users_last_seen_at_not_null" CHECK (last_seen_at IS NOT NULL),
...
CREATE TABLE "public"."api_keys" (
  ...
  CONSTRAINT "api_keys_last_used_not_before_created_at" CHECK (last_used_at >= created_at),
  ...
  CONSTRAINT "api_keys_last_used_not_in_future_utc" CHECK (last_used_at <= (now() AT TIME ZONE 'UTC')),
...
CREATE TABLE "public"."replicas" (
  ...
  CONSTRAINT "replicas_stopped_at_not_before_started_at" CHECK (stopped_at IS NULL OR stopped_at >= started_at)
);
...
CREATE TABLE "public"."workspace_proxies" (
  ...
  CONSTRAINT "workspace_proxies_url_requires_http_https_scheme" CHECK (url ~ '^https?://')
);
...
CREATE TABLE "public"."workspace_agents" (
  ...
  CONSTRAINT "workspace_agents_startup_logs_length_non_negative" CHECK (startup_logs_length >= 0),
...
CREATE TABLE "public"."workspace_builds" (
  ...
  CONSTRAINT "workspace_builds_build_number_non_negative" CHECK (build_number >= 0)
);
