...
CREATE TABLE `users` (
    ...
    `email` varchar(255) COLLATE utf8mb4_0900_as_ci NOT NULL COMMENT 'Email address of the user, now stored case-insensitively',
    ...
    `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'Timestamp of the last update to the user record',
    ...
    INDEX `users_email_lower` ((LOWER(`email`))),
...
CREATE TABLE `categories` (
    ...
    `category_code` varchar(10) NOT NULL UNIQUE COMMENT 'Unique code for the category of a fixed length',
    ...
    `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'Timestamp of the last update to the category record',
...
CREATE TABLE `products` (
    ...
    INDEX `products_manufacturer_status` (`manufacturer`, `status`),
    INDEX `products_price` (`price`),
    INDEX `products_deleted_at` (`deleted_at`),
    INDEX `products_category_id_price` (`category_id`, `price`),
...
CREATE TABLE `orders` (
    ...
    `total_amount` decimal(12,2) NOT NULL COMMENT 'Total amount for the order',
    ...
    INDEX `orders_fulfillment_center_id_order_status_created_at` (`fulfillment_center_id`, `order_status`, `created_at`),
    UNIQUE INDEX `order_reference` (`order_reference`),
    ...
    INDEX `orders_user_id_order_reference_covering` (`user_id`, `order_reference`),
    INDEX `orders_user_id_created_at_history` (`user_id`, `created_at`),
    INDEX `orders_order_reference_created_at_lookup` (`order_reference`, `created_at`),
    INDEX `orders_user_id_fulfillment_center_id_created_at` (`user_id`, `fulfillment_center_id`, `created_at`),
    INDEX `orders_order_reference_created_at` (`order_reference`, `created_at`),
    INDEX `orders_user_id_order_status_created_at_history` (`user_id`, `order_status`, `created_at`),
    INDEX `orders_order_reference_created_at_opt` (`order_reference`, `created_at`),
    INDEX `orders_order_status_created_at_user_id` (`order_status`, `created_at`, `user_id`),
    INDEX `orders_order_reference_created_at_new` (`order_reference`, `created_at`),
